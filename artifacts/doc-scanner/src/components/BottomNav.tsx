...
export function BottomNav({ onSearchClick, onSettingsClick, onFoldersClick }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-card border-t border-border pb-[env(safe-area-inset-bottom)]">
      <div className="relative flex items-stretch justify-between px-2 max-w-lg mx-auto">
        <NavItem
        ...
        {/* Center slot: label stays in normal flow so it stays aligned with
            the other tabs; the raised circle is absolutely positioned above
            it so it doesn't affect row alignment. */}
        <Link
          href="/capture"
          data-testid="nav-scan"
          className="relative flex flex-col items-center justify-end gap-1 flex-1 py-2"
        >
          <div
            className="absolute -top-7 left-1/2 -translate-x-1/2 w-16 h-16 rounded-full bg-primary text-primary-foreground flex items-center justify-center shadow-lg ring-4 ring-card transition-transform active:scale-95 hover:scale-105"
...
function NavItem({
  ...
  if (asLink) {
    return (
      <Link href={asLink} data-testid={`nav-${label.toLowerCase()}`} className="flex-1 flex">
        {content}
      </Link>
    );
  }
  ...
  return (
    <button onClick={onClick} data-testid={`nav-${label.toLowerCase()}`} className="flex-1 flex">
      {content}
    </button>
  );
}
